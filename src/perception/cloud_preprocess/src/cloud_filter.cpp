...
float box_x_min_inside = -6, box_x_max_inside = 0.2;
...
PointCloud Cloud_PassThroughInside(PointCloud cp_src,int leida)
{
    if(leida == 2 || leida == 3)
    {
        box_y_min_inside = -0.07;       //将自己的蒙皮滤掉
...
int main(int argc, char **argv)
{
    ...
    cloud_pub1 = nh.advertise<sensor_msgs::PointCloud2>("filter_leida1", 10);   
    cloud_pub2 = nh.advertise<sensor_msgs::PointCloud2>("filter_leida2", 10);
    cloud_pub3 = nh.advertise<sensor_msgs::PointCloud2>("filter_leida3", 10);
