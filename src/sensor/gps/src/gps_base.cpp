...
private:
    ...
    ros::Publisher gps_pub, gps_raw_pub;
...
int TGPS_Base::DataProc(const char *buf)
{
    ...
    if (res) //  必须有角度
    {
        ...
        msg.raw_UTM_Y = gdata.UTM_Y, msg.raw_UTM_X = gdata.UTM_X;
        msg.raw_Angle = 270-gdata.Angle;
        if (msg.raw_Angle>=180)  msg.raw_Angle-=360;
        else if (msg.raw_Angle<-180)  msg.raw_Angle+=360;
