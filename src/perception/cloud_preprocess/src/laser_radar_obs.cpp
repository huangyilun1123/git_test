...
class LaserRadarObs
{
    private:
        ...
        PointCloud Cloud_Pass_Through(const PointCloud &cp_src,
            float x_min, float x_max, float y_min, float y_max, float z_min, float z_max);//矩形滤波滤外侧
        PointCloud Cloud_Pass_Through_Inside(const PointCloud &cp_src,
            float x_min, float x_max, float y_min, float y_max, float z_min, float z_max);//矩形滤波滤内侧
        PointCloud Cloud_Pass_Through_Redius(const PointCloud &cp_src, float max_redius);//距离滤波
        vector<pcl::PointIndices> Clould_Cluster(const PointCloud &cp_src, 
            float cluster_distance, int cluster_number_min, int cluster_number_max);//激光雷达聚类
...
LaserRadarObs::LaserRadarObs()
{
    ...
    laser_point_cloud_sub = 
        nh->subscribe<sensor_msgs::PointCloud2>("/laserscan_check_around/cloud_around", 10, 
        &LaserRadarObs::LaserscanPointCloudCallback, this);
    car_cmd_sub = 
        nh->subscribe<data_comm::car_ctr>("/pathtrack/ctr_cmd", 10, 
        &LaserRadarObs::CarCmdCallBack, this);
    local_path_sub = 
        nh->subscribe<nav_msgs::Path>("/local_path_plan/localpath", 10,
        &LaserRadarObs::LocalPathCallBack, this);
...
PointCloud LaserRadarObs::Cloud_Pass_Through(const PointCloud &cp_src, 
    float x_min, float x_max, float y_min, float y_max, float z_min = -5, float z_max = 5)
{
    PointCloud res;
...
PointCloud LaserRadarObs::Cloud_Pass_Through_Inside(const PointCloud &cp_src,
    float x_min, float x_max, float y_min, float y_max, float z_min = -5, float z_max = 5)
{
    PointCloud res;
...
vector<pcl::PointIndices> LaserRadarObs::Clould_Cluster(const PointCloud &cp_src, 
    float cluster_distance, int cluster_number_min, int cluster_number_max)
{
...
{
    ros::init(argc, argv, "laser_radar_obs");
    LaserRadarObs lro;
    ros::spin();
}